...
def _extract_tokens_and_drawnos_from_page(page_text: str) -> Tuple[List[int], List[int]]:
    ...
    for ln in lines:
        if "Lotto" in ln and "6/49" in ln: continue
        if "multipasko" in ln.lower(): continue
        if "www." in ln.lower(): continue
        if "©" in ln: continue
...
def smart_ok(
    ...
    nums = sorted(ticket)
    if block_run_3 and has_run_length(nums, 3): return False
    if block_run_2 and has_run_length(nums, 2): return False
    pairs = count_adjacent_pairs(nums)
    if max_adjacent_pairs is not None and pairs > max_adjacent_pairs: return False
...
def generate_with_smart_filters(
    ...
    while len(out) < n_tickets:
        attempts += 1
        if attempts > n_tickets * max_attempts_per_ticket: break
        rec = gen_func()
...
def pick_daily_set_from_hot(
    hot: List[int], pick_count: int, nmin: int, nmax: int,
    prefer_parity: str, prefer_level: str, threshold: int,
    target_spread: Optional[float] = None, max_attempts: int = 650
) -> List[int]:
    ...
    if prefer_level != "ANY":
        ...
        if len(filtered) >= pick_count: pool = filtered
...
def build_positional_difference_set(draws: List[List[int]], window: int) -> Dict:
    ...
    for pos in range(PICK_COUNT):
        ...
        for prev in previous_draws:
            if len(prev) != PICK_COUNT: continue
...
# =========================================================
# NEW STRONGER ENGINE LOGIC
# =========================================================
def run_new_stronger_engine(
    draws: List[List[int]],
    n_tickets: int,
    candidate_count: int,
    hot_size: int,
    ...
    pair_counter, triple_counter = compute_pair_triple_stats_cached(draws)
    target_profile = build_target_profile(draws)
    recent_draws = draws[:10]

    full_pool = list(range(NUM_MIN, NUM_MAX + 1))
    hot_pool = sorted(percent_df.head(hot_size)["Liczba"].tolist())
    ...
    while len(candidates) < candidate_count and attempts < max_attempts:
        attempts += 1
        hot_part = random.sample(hot_pool, min(hot_count, len(hot_pool)))
        rem_pool = [x for x in full_pool if x not in hot_part]
        other_part = random.sample(rem_pool, PICK_COUNT - len(hot_part))
        ticket = tuple(sorted(hot_part + other_part))
...
def build_turbo_score_ranking(
    draws_for_window: List[List[int]], hot: List[int], cold: List[int],
    base_mode_kind: str, mix_hot_count: int, candidate_count: int, top_n: int
) -> Dict:
    ...
    candidates = generate_candidate_tickets(candidate_count, base_mode_kind, hot, cold, mix_hot_count)
    scored = []
    for ticket in candidates:
        scored.append(score_ticket(ticket, percent_map, pair_counter, triple_counter, target_profile, recent_draws))
    ...
    return {"rows": rows, "target_profile": target_profile, "candidate_count_used": len(candidates)}
...
def build_premium_ranking(
    draws_for_window: List[List[int]], hot: List[int], cold: List[int],
    mix_hot_count: int, candidate_count: int, top_n: int
) -> Dict:
    percent_df = compute_presence_percent_df_cached(draws_for_window)
    percent_map = dict(zip(percent_df["Liczba"], percent_df["Procent_losowan"]))
    pair_counter, triple_counter = compute_pair_triple_stats_cached(draws_for_window)
    ...
    turbo_seed = build_turbo_score_ranking(
        draws_for_window=draws_for_window, hot=hot, cold=cold, base_mode_kind="hybrid",
        mix_hot_count=mix_hot_count, candidate_count=max(100, candidate_count // 2), top_n=min(10, top_n * 2)
    )
    ...
    ordinary_candidates = generate_candidate_tickets(max(candidate_count, 200), "hybrid", hot, cold, mix_hot_count)
    ...
    premium_scored = []
    for ticket in uniq:
        base = score_ticket(ticket, percent_map, pair_counter, triple_counter, target_profile, recent_draws)
        ...
        premium_bonus = (overlap_hot_max * 5.0 + overlap_diff * 3.0 + overlap_hot * 1.2)
    ...
    return {
        "rows": rows, "candidate_count_used": len(uniq),
        "hot_max_set": hot_max_set, "hot_max_table": hot_max_table,
        "diff_set": diff_set, "diff_details": diff_data["details"],
...
def run_ai_simulation_cached(percent_df: pd.DataFrame, n_sims: int) -> Dict:
    ...
    for _ in range(n_sims):
        ...
        for n in draw: number_occ[n] += 1
        for pair in combinations(draw, 2): pair_occ[pair] += 1
    ...
    top_pairs = pd.DataFrame([{"Para": f"{a:02d}-{b:02d}", "Wystąpienia": c} for (a, b), c in pair_occ.most_common(15)])

    return {"sim_df": sim_df, "sim_set": sim_set, "top_pairs": top_pairs, "n_sims": n_sims}
...
def detect_cycles_cached(draws: List[List[int]]) -> pd.DataFrame:
    ...
    for num in range(NUM_MIN, NUM_MAX + 1):
        ...
        for idx, draw in enumerate(chronological):
            if num in draw: positions.append(idx)
        ...
        if occurrences >= 2:
            ...
            cycle_ratio = 0.0
        rows.append({
            "Liczba": num, "Wystąpienia": occurrences, "Średni_cykl": round(avg_gap, 2),
            "Aktualna_przerwa": round(last_gap, 2), "Cycle_Ratio": round(cycle_ratio, 3)
        })
    df = pd.DataFrame(rows).sort_values(["Cycle_Ratio", "Wystąpienia", "Liczba"], ascending=[False, False, True]).reset_index(drop=True)
    return df
...
def make_txt_for_hot_master_set(hot_master_set: List[int], history_window: int) -> bytes:
    ...
    text = (f"Zestaw 6 najczęściej padających liczb wg procentu losowań\nAnalizowana historia: ostatnie {history_window} losowań\nZestaw: {nums}\n")
    return text.encode("utf-8")

def make_txt_for_difference_set(diff_data: Dict, selected_window: int) -> bytes:
    nums = " ".join(f"{x:02d}" for x in diff_data["set"])
    lines = [
        "Zestaw różnic pozycyjnych", f"Wybrany zakres użytkownika: {selected_window}",
        f"Faktycznie użyty zakres: {diff_data['window_used']}", f"Zestaw: {nums}", "", "Szczegóły pozycji:"
    ]
    for d in diff_data["details"]:
        lines.append(f"Pozycja {d['Pozycja']}: najnowsza={d['Najnowsza liczba']}, najczęstsza różnica={d['Najczęstsza różnica']}, ile razy={d['Ile razy']}, wybrana={d['Wybrana liczba']}")
    return ("\n".join(lines) + "\n").encode("utf-8")
...
def make_txt_for_hot_max_set(hot_max_set: List[int], hot_max_table: pd.DataFrame, selected_window: int) -> bytes:
    ...
    lines = ["HOT MAX 6 — zestaw 6 najczęściej padających liczb wg procentu losowań", f"Zakres analizy: ostatnie {selected_window} losowań", f"Zestaw: {nums}", "", "TOP 6:"]
    for _, row in hot_max_table.iterrows():
        lines.append(f"Liczba {int(row['Liczba']):02d} | Losowania z wystąpieniem: {int(row['Liczba_losowan_z_wystapieniem'])} | Procent: {float(row['Procent_losowan']):.2f}%")
    return ("\n".join(lines) + "\n").encode("utf-8")

def make_txt_for_turbo_score(rows: List[Dict], candidate_count_used: int) -> bytes:
    lines = ["Turbo Score — ranking kuponów", f"Liczba ocenionych kandydatów: {candidate_count_used}", ""]
    for row in rows:
        lines.append(f"#{row['Ranking']} | {row['Kupon']} | Score={row['Score']} | Liczby={row['Score liczb']} | Pary={row['Score par']} | Trójki={row['Score trójek']} | Parzyste/Nieparzyste={row['Parzyste/Nieparzyste']} | Niskie/Wysokie={row['Niskie/Wysokie']} | Rozstrzał={row['Rozstrzał']} | Pary kolejne={row['Pary kolejne']} | Podobieństwo do ostatnich={row['Podobieństwo do ostatnich']}")
    return ("\n".join(lines) + "\n").encode("utf-8")

def make_txt_for_premium(rows: List[Dict], candidate_count_used: int) -> bytes:
    lines = ["Premium Mode — ranking kuponów", f"Liczba ocenionych kandydatów: {candidate_count_used}", ""]
    for row in rows:
        lines.append(f"#{row['Ranking']} | {row['Kupon']} | Premium Score={row['Premium Score']} | Base Score={row['Bazowy Score']} | Bonus={row['Bonus Premium']} | HOT MAX trafień={row['HOT MAX trafień']} | Różnice trafień={row['Różnice trafień']} | Hot trafień={row['Hot trafień']} | Parzyste/Nieparzyste={row['Parzyste/Nieparzyste']} | Niskie/Wysokie={row['Niskie/Wysokie']} | Rozstrzał={row['Rozstrzał']} | Pary kolejne={row['Pary kolejne']} | Podobieństwo do ostatnich={row['Podobieństwo do ostatnich']}")
    return ("\n".join(lines) + "\n").encode("utf-8")

def make_txt_for_simulation(sim: Dict) -> bytes:
    nums = " ".join(f"{x:02d}" for x in sim["sim_set"])
    lines = [f"AI symulacja Lotto — liczba symulacji: {sim['n_sims']}", f"Sugerowany zestaw: {nums}", "", "TOP pary:"]
...
def make_txt_for_cycles(cycle_df: pd.DataFrame, cycle_set: List[int]) -> bytes:
    ...
    lines = ["Lotto — analiza cykli liczb", f"Sugerowany zestaw cykli: {nums}", "", "TOP liczby cykliczne:"]
    for _, row in cycle_df.head(15).iterrows():
        lines.append(f"Liczba {int(row['Liczba']):02d} | Wystąpienia={int(row['Wystąpienia'])} | Średni cykl={row['Średni_cykl']} | Aktualna przerwa={row['Aktualna_przerwa']} | Cycle ratio={row['Cycle_Ratio']}")
    return ("\n".join(lines) + "\n").encode("utf-8")

def make_txt_for_wielka_szansa(ws: Dict, selected_window: int) -> bytes:
    nums = " ".join(f"{x:02d}" for x in ws["set"])
    lines = ["Wielka Szansa — prognoza wykresowa pozycji 1–6", f"Zakres analizy: {selected_window}", f"Faktycznie użyty zakres: {ws['window_used']}", f"Zestaw: {nums}", "", ws["opis"], "", "Szczegóły pozycji:"]
    for d in ws["details"]:
...
**📋 Pokaż wyniki** Pokazuje ostatnie wyniki wczytane z PDF.

**🔥 Zestaw 6 HOT** Pokazuje 6 liczb o najwyższym procencie wystąpień w losowaniach z wybranego zakresu.

**📐 Zestaw różnic** Analizuje różnice pozycji 1–6 między najnowszym losowaniem a wcześniejszymi.

**⭐ Turbo Score / Ranking kuponów** Generuje wielu kandydatów, punktuje ich i wybiera najlepsze kupony na podstawie statystyk.

**👑 Premium** Łączy HOT/COLD, różnice pozycyjne, Turbo Score oraz mutacje dla topowego typowania.

**🤖 AI symulacja** Ważone symulacje losowań na podstawie historycznych procentów.

**🔄 Cykle liczb** Szuka liczb, które historycznie miały pewien rytm i są blisko swojego cyklu.

**🎯 Wielka Szansa** Prognoza oparta o "wykresy" każdej z 6 pozycji.
        """)
...
def settings_panel(defaults: Dict) -> Dict:
    ...
    if engine_choice == "Nowy mocniejszy silnik (rankingowy)":
        ...
        with c_b:
            ne_hs = st.slider("Pula gorących liczb", 10, 30, 18, 1)
        with c_c:
            ne_hc = st.slider("Ile liczb z gorących?", 1, 6, 4, 1)
    else:
        ne_cand = 3000
        ne_hs = 18
        ne_hc = 4
    ...
    mode_ui = st.selectbox(
        ...
        ["Hybryda 70/20/10 (hot/cold/mix)", "Tylko 🔥 gorące", "Tylko ❄️ zimne", "Tylko ⚗️ mix (hot+zimne)", "Premium 👑"],
        index=defaults.get("mode_index", 0)
    )

    history_window = st.selectbox("Ile ostatnich losowań brać do analizy HOT/COLD?", [50, 100, 250, 500, 750, 1000], index=defaults.get("hist_index", 5))
    difference_window = st.selectbox("Analiza różnic pozycyjnych — zakres losowań", [50, 100, 250, 500, 750, 999], index=defaults.get("diff_hist_index", 5))
    wielka_szansa_window = st.selectbox("Wielka Szansa — zakres wykresu pozycji", [100, 999], index=defaults.get("wielka_idx", 1))
    ...
    hot_max_enabled = st.checkbox("Włącz HOT MAX 6 (działa tylko przy trybie: Tylko gorące)", value=defaults.get("hot_max_enabled", False))
    hot_max_count = st.selectbox("Ile cyfr z HOT MAX?", [1, 2, 3, 4, 5, 6], index=defaults.get("hot_max_count_idx", 5))

    st.markdown("---")
    st.subheader("⭐ Turbo Score / Ranking kuponów")
    turbo_candidate_count = st.selectbox("Ile kandydatów ma ocenić Turbo Score?", [100, 200, 300, 500, 750, 1000], index=defaults.get("turbo_candidate_idx", 3))
    turbo_top_n = st.selectbox("Ile najlepszych kuponów pokazać?", [3, 5, 10, 15, 20], index=defaults.get("turbo_top_idx", 4))

    st.markdown("---")
    st.subheader("👑 Premium")
    premium_candidate_count = st.selectbox("Premium: ile kandydatów ma zbudować silnik premium?", [200, 300, 500, 750, 1000, 1500], index=defaults.get("premium_candidate_idx", 2))
    premium_top_n = st.selectbox("Premium: ile finalnych kuponów pokazać?", [3, 5, 10, 15, 20], index=defaults.get("premium_top_idx", 4))

    st.markdown("---")
    st.subheader("🤖 AI symulacja")
    ai_sim_count = st.selectbox("Ile losowań ma zasymulować AI?", [5000, 10000, 25000, 50000, 100000], index=defaults.get("ai_sim_idx", 4))

    st.markdown("---")
    st.subheader("🧠 Tryb inteligentny (dla starej metody)")
    ...
    if smart_enabled:
        ...
        even_odd_choice = st.radio("Parzyste / Nieparzyste (6 liczb)", ["Dowolnie", "3/3", "4/2", "2/4", "5/1", "1/5", "6/0", "0/6"], index=defaults.get("even_odd_idx", 0))
    ...
    return {
        ...
        "new_engine_hot_size": ne_hs,
        "new_engine_hot_count": ne_hc,
        "mode_ui": mode_ui,
...
# =========================================================
def main():
    st.set_page_config(page_title="Generator-Victory Lotto", page_icon="🏆", layout="wide", initial_sidebar_state="collapsed")
    ...
    st.markdown('<div class="v-muted">Gorące = najwyższy procent losowań z wystąpieniem liczby. Zimne = najniższy procent losowań z wystąpieniem liczby.</div>', unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    defaults = {
        "engine_choice_idx": 1, "mode_index": 0, "hist_index": 5, "diff_hist_index": 5, "wielka_idx": 1,
        "n_tickets": 50, "hot_size": 20, "cold_size": 20, "mix_hot_count": 3, "preview_limit": 60,
        "hot_max_enabled": False, "hot_max_count_idx": 5, "turbo_candidate_idx": 3, "turbo_top_idx": 4,
        "premium_candidate_idx": 2, "premium_top_idx": 4, "ai_sim_idx": 4, "smart_enabled": False,
        "block_run_2": True, "block_run_3": True, "limit_pairs_on": True, "max_adj_pairs": 2,
        "even_odd_idx": 0, "max_attempts": 120
    }
    ...
    mode_ui = cfg["mode_ui"]
    base_mode_kind = "mix"
    if mode_ui == "Hybryda 70/20/10 (hot/cold/mix)": base_mode_kind = "hybrid"
    elif mode_ui == "Tylko 🔥 gorące": base_mode_kind = "hot"
    elif mode_ui == "Tylko ❄️ zimne": base_mode_kind = "cold"
    elif mode_ui == "Premium 👑": base_mode_kind = "premium"

    hot_max_mode_active = (base_mode_kind == "hot" and cfg["hot_max_enabled"] and cfg["hot_max_count"] == 6)

    if build_turbo:
        turbo_result = build_turbo_score_ranking(
            draws_for_window=draws, hot=hot, cold=cold,
            base_mode_kind="hybrid" if base_mode_kind == "premium" else base_mode_kind,
            mix_hot_count=cfg["mix_hot_count"], candidate_count=cfg["turbo_candidate_count"], top_n=cfg["turbo_top_n"]
        )
    ...
    if build_premium or base_mode_kind == "premium":
        premium_result = build_premium_ranking(
            draws_for_window=draws, hot=hot, cold=cold, mix_hot_count=cfg["mix_hot_count"],
            candidate_count=cfg["premium_candidate_count"], top_n=cfg["premium_top_n"]
        )
        st.session_state["premium_result"] = premium_result

    if build_ai_sim:
        st.session_state["ai_sim_result"] = run_ai_simulation_cached(percent_df=percent_df, n_sims=cfg["ai_sim_count"])

    if build_cycles:
        cycle_df = detect_cycles_cached(draws)
        st.session_state["cycle_result"] = {"df": cycle_df, "set": sorted(cycle_df.head(PICK_COUNT)["Liczba"].tolist())}
    ...
    def gen_one_record() -> Dict:
        if base_mode_kind == "premium":
            premium_result_local = build_premium_ranking(
                draws_for_window=draws, hot=hot, cold=cold, mix_hot_count=cfg["mix_hot_count"],
                candidate_count=cfg["premium_candidate_count"], top_n=max(3, min(cfg["premium_top_n"], 20))
            )
            best_row = premium_result_local["rows"][0]
            return {"Typ": "premium", "Kupon": [int(x) for x in best_row["Kupon"].split()]}
        if hot_max_mode_active:
            hot_max_set, hot_max_table = gen_ticket_hot_max_percent(draws)
            return {"Typ": "hot_max_6", "Kupon": hot_max_set, "HotMaxTable": hot_max_table}
        if base_mode_kind == "hybrid":
            chosen = random.choices(["hot", "cold", "mix"], weights=[HYBRID_HOT_P, HYBRID_COLD_P, HYBRID_MIX_P], k=1)[0]
    ...
    if generate:
        ...
        if cfg["engine_choice"] == "Nowy mocniejszy silnik (rankingowy)":
            with st.spinner("Nowy silnik losuje i ocenia kandydatów..."):
                recs = run_new_stronger_engine(
                    draws=draws,
                    n_tickets=int(cfg["n_tickets"]),
                    candidate_count=cfg["new_engine_candidates"],
                    hot_size=cfg["new_engine_hot_size"],
                    hot_count=cfg["new_engine_hot_count"]
                )
                progress.progress(100)
                status.write(f"Wygenerowano {len(recs)} najlepszych kuponów Nowym Silnikiem!")
        else:
            with st.spinner("Generuję kupony stara metodą..."):
                if base_mode_kind == "premium":
                    premium_result_local = build_premium_ranking(
                        draws_for_window=draws, hot=hot, cold=cold, mix_hot_count=cfg["mix_hot_count"],
                        candidate_count=cfg["premium_candidate_count"], top_n=cfg["premium_top_n"]
                    ...
                    for i in range(total):
                        recs.append({"Typ": "premium", "Kupon": [int(x) for x in premium_rows[i]["Kupon"].split()]})
                    ...
                    for i in range(total):
                        recs.append({"Typ": "hot_max_6", "Kupon": hot_max_set, "HotMaxTable": hot_max_table})
                        if (i + 1) % 10 == 0 or (i + 1) == total:
                            progress.progress(int((i + 1) / total * 100))
                            status.write(f"Postęp: {i+1}/{total}")
                    st.session_state["hot_max_set"] = {"set": hot_max_set, "table": hot_max_table, "window": history_window_used}
                else:
                    if not cfg["smart_enabled"]:
                        ...
                        smart_kwargs = {
                            "block_run_2": cfg["block_run_2"], "block_run_3": cfg["block_run_3"],
                            "max_adjacent_pairs": cfg["max_adj_pairs"], "even_odd_choice": cfg["even_odd_choice"]
                        }
                        recs = generate_with_smart_filters(
                            gen_func=gen_one_record, n_tickets=int(cfg["n_tickets"]),
                            max_attempts_per_ticket=int(cfg["max_attempts_per_ticket"]), smart_kwargs=smart_kwargs
                        )
                        progress.progress(100)
                        status.write(f"Postęp: {len(recs)}/{int(cfg['n_tickets'])}")

            if cfg["engine_choice"] != "Nowy mocniejszy silnik (rankingowy)" and cfg["smart_enabled"] and (not hot_max_mode_active) and base_mode_kind != "premium" and len(recs) < int(cfg["n_tickets"]):
                st.warning(f"⚠️ Filtry są ostre: wygenerowano **{len(recs)}** / {int(cfg['n_tickets'])} kuponów. Poluzuj filtry albo zwiększ limit prób.")
    ...
    if daily:
        ...
        daily_set = pick_daily_set_from_hot(
            hot=hot, pick_count=PICK_COUNT, nmin=NUM_MIN, nmax=NUM_MAX,
            prefer_parity=prefer_parity, prefer_level=prefer_level, threshold=24,
            target_spread=target_spread, max_attempts=650
        )
        st.session_state["last_daily"] = {"set": daily_set, "prefer_parity": prefer_parity, "prefer_level": prefer_level, "target_spread": target_spread}
    ...
    if st.session_state["show_results"]:
        ...
        st.download_button("⬇️ Pobierz wyniki jako TXT", data=make_txt_for_results(slice_records), file_name=results_name, mime="text/plain", use_container_width=True)
    ...
    if st.session_state.get("hot_master_set") is not None:
        ...
        st.markdown(f'<div class="v-row"><b>Zestaw HOT 6</b> — {hot_set_str} <span class="v-muted"> | wyliczone z ostatnich {history_window_used} losowań według najwyższego % wystąpień</span></div>', unsafe_allow_html=True)
        hot_set_name = sanitize_txt_filename(st.text_input("Nazwa pliku zestawu HOT .txt", value="hot6.txt"))
        st.download_button("⬇️ Pobierz zestaw HOT 6 jako TXT", data=make_txt_for_hot_master_set(hot_set, history_window_used), file_name=hot_set_name, mime="text/plain", use_container_width=True)
    ...
    if st.session_state.get("difference_set") is not None:
        ...
        st.markdown(f'<div class="v-row"><b>Zestaw różnic</b> — {diff_set_str} <span class="v-muted"> | zakres użytkownika: {cfg["difference_window"]} | faktycznie użyto: {diff_data["window_used"]}</span></div>', unsafe_allow_html=True)
        ...
        st.download_button("⬇️ Pobierz zestaw różnic jako TXT", data=make_txt_for_difference_set(diff_data, cfg["difference_window"]), file_name=diff_name, mime="text/plain", use_container_width=True)
    ...
    if st.session_state.get("wielka_szansa_result") is not None:
        ...
        st.markdown(f'<div class="v-row-danger"><b>Prognoza wykresowa</b> — {ws_str} <span class="v-muted"> | zakres: {cfg["wielka_szansa_window"]} | użyto: {ws["window_used"]}</span></div>', unsafe_allow_html=True)
        ...
        st.download_button("⬇️ Pobierz Wielką Szansę jako TXT", data=make_txt_for_wielka_szansa(ws, cfg["wielka_szansa_window"]), file_name=ws_name, mime="text/plain", use_container_width=True)

    if st.session_state.get("hot_max_set") is not None:
        hms = st.session_state["hot_max_set"]
        hot_max_str = " ".join(f"{x:02d}" for x in hms["set"])
        st.markdown("### 🔥 HOT MAX 6 — zestaw procentowy")
        st.markdown(f'<div class="v-row"><b>HOT MAX 6</b> — {hot_max_str} <span class="v-muted"> | zakres: ostatnie {hms["window"]} losowań | tryb: tylko gorące + HOT MAX 6</span></div>', unsafe_allow_html=True)
        st.markdown("#### TOP 6 według procentu losowań")
        hot_max_table_display = hms["table"].copy()
        ...
        st.download_button("⬇️ Pobierz HOT MAX 6 jako TXT", data=make_txt_for_hot_max_set(hms["set"], hms["table"], hms["window"]), file_name=hot_max_name, mime="text/plain", use_container_width=True)
    ...
    if st.session_state.get("turbo_score_result") is not None:
        ...
        st.markdown(f'<div class="v-row"><b>Turbo Score</b> — oceniono {turbo["candidate_count_used"]} kandydatów i wybrano TOP {len(turbo["rows"])}</div>', unsafe_allow_html=True)
        ...
        st.markdown(f'<div class="v-row"><b>Balans parzyste/nieparzyste:</b> {profile["target_even_odd"][0]}/{profile["target_even_odd"][1]} | <b>Balans niskie/wysokie:</b> {profile["target_low_high"][0]}/{profile["target_low_high"][1]} | <b>Średni rozstrzał:</b> {profile["target_spread"]:.2f} | <b>Średnia liczba par kolejnych:</b> {profile["target_pairs"]:.2f}</div>', unsafe_allow_html=True)
        turbo_name = sanitize_txt_filename(st.text_input("Nazwa pliku Turbo Score .txt", value="turbo_score.txt"))
        st.download_button("⬇️ Pobierz ranking Turbo Score jako TXT", data=make_txt_for_turbo_score(turbo["rows"], turbo["candidate_count_used"]), file_name=turbo_name, mime="text/plain", use_container_width=True)
    ...
    if st.session_state.get("premium_result") is not None:
        ...
        st.markdown(f'<div class="v-row-premium"><b>Premium Mode</b> — oceniono {premium["candidate_count_used"]} kandydatów i wybrano TOP {len(premium["rows"])}</div>', unsafe_allow_html=True)
        ...
        st.markdown(f'<div class="v-row-premium"><b>Źródła premium:</b><br>HOT MAX 6: {hot_max_str}<br>Zestaw różnic: {diff_str}</div>', unsafe_allow_html=True)
        profile = premium["target_profile"]
        st.markdown(f'<div class="v-row-premium"><b>Profil docelowy premium:</b><br>Parzyste/Nieparzyste: {profile["target_even_odd"][0]}/{profile["target_even_odd"][1]}<br>Niskie/Wysokie: {profile["target_low_high"][0]}/{profile["target_low_high"][1]}<br>Średni rozstrzał: {profile["target_spread"]:.2f}<br>Średnia liczba par kolejnych: {profile["target_pairs"]:.2f}</div>', unsafe_allow_html=True)
        premium_name = sanitize_txt_filename(st.text_input("Nazwa pliku Premium .txt", value="premium_mode.txt"))
        st.download_button("⬇️ Pobierz ranking Premium jako TXT", data=make_txt_for_premium(premium["rows"], premium["candidate_count_used"]), file_name=premium_name, mime="text/plain", use_container_width=True)
    ...
    if st.session_state.get("ai_sim_result") is not None:
        ...
        st.markdown(f'<div class="v-row-blue"><b>Symulacje:</b> {sim["n_sims"]} | <b>Sugerowany zestaw:</b> {sim_str}</div>', unsafe_allow_html=True)
        ...
        st.download_button("⬇️ Pobierz wynik AI symulacji jako TXT", data=make_txt_for_simulation(sim), file_name=sim_name, mime="text/plain", use_container_width=True)
    ...
    if st.session_state.get("cycle_result") is not None:
        ...
        st.markdown(f'<div class="v-row-blue"><b>Sugerowany zestaw cykli:</b> {cycle_set_str}</div>', unsafe_allow_html=True)
        ...
        st.download_button("⬇️ Pobierz analizę cykli jako TXT", data=make_txt_for_cycles(cyc["df"], cyc["set"]), file_name=cycle_name, mime="text/plain", use_container_width=True)
    ...
    if st.session_state.get("last_daily") is not None:
        ...
        pref_to_text = lambda p: "parzyste" if p == "EVEN" else ("nieparzyste" if p == "ODD" else "dowolnie")
        level_to_text = lambda p: "niższe" if p == "LOW" else ("wyższe" if p == "HIGH" else "dowolnie")
        st.markdown("### 🌿 Twoje szczęśliwe cyfry dnia")
        st.markdown(f'<div class="v-row"><b>Zestaw dnia</b> — {nums} <span class="v-muted"> | parzyste/nieparzyste: {ev}/{od} | pary: {pairs}</span></div>', unsafe_allow_html=True)
    ...
    if records:
        ...
        st.download_button("⬇️ Pobierz kupony jako TXT", data=make_txt_for_tickets(records), file_name=tickets_name, mime="text/plain", use_container_width=True)
    ...
    with st.expander("📌 Diagnostyka procentowa (TOP/LOW)"):
        ...
        low_display = percent_df.tail(15).sort_values(["Procent_losowan", "Liczba_losowan_z_wystapieniem", "Liczba"], ascending=[True, True, True]).copy()
